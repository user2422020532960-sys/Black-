...
async function handleAIMessage({ api, event, userMsg, message, commandName, senderID, threadID }) {
  ...
  try {
    const truncatedMsg = userMsg.length > MAX_MSG_LENGTH
      ? userMsg.slice(0, MAX_MSG_LENGTH) + "…"
      : userMsg;

    await fetchUserName(api, senderID);
    detectNameFromText(truncatedMsg, senderID);
    extractFacts(truncatedMsg, senderID, threadID);

    const profile = getProfile(senderID);
    const genderDetected = detectGenderFromText(truncatedMsg);
