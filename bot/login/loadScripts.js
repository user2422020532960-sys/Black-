        ...
        for (const folderModules of folders) {
                ...
                for (const file of Files) {
                        ...
                        try {
                                ...
                                if (onChat) {
                                        const idx = BlackBot.onChat.indexOf(commandName);
                                        if (idx !== -1) BlackBot.onChat.splice(idx, 1);
                                        BlackBot.onChat.push(commandName);
                                }
                                if (onFirstChat) {
                                        const idx = BlackBot.onFirstChat.findIndex(o => o.commandName === commandName);
                                        if (idx !== -1) BlackBot.onFirstChat.splice(idx, 1);
                                        BlackBot.onFirstChat.push({ commandName, threadIDsChattedFirstTime: [] });
                                }
                                if (onEvent) {
                                        const idx = BlackBot.onEvent.indexOf(commandName);
                                        if (idx !== -1) BlackBot.onEvent.splice(idx, 1);
                                        BlackBot.onEvent.push(commandName);
                                }
                                if (onAnyEvent) {
                                        const idx = BlackBot.onAnyEvent.indexOf(commandName);
                                        if (idx !== -1) BlackBot.onAnyEvent.splice(idx, 1);
                                        BlackBot.onAnyEvent.push(commandName);
                                }
