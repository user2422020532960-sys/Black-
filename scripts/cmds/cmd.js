...
const { configCommands } = global.BlackBot;
...
function loadScripts(folder, fileName, log, configCommands, api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, getLang, rawCode) {
	// global.BlackBot[folderModules == "cmds" ? "commandFilesPath" : "eventCommandsFilesPath"].push({
	// 	filePath: pathCommand,
	// 	commandName: [commandName, ...validAliases]
	// });
	const storageCommandFilesPath = global.BlackBot[folder == "cmds" ? "commandFilesPath" : "eventCommandsFilesPath"];
	...
	try {
		...
		const { onFirstChat: allOnFirstChat, onChat: allOnChat, onEvent: allOnEvent, onAnyEvent: allOnAnyEvent } = BlackBot;
		...
		// —————————————— CHECK COMMAND EXIST ——————————————— //
		if (!oldCommandName) {
			if (BlackBot[setMap].get(oldCommandName)?.location != pathCommand)
				throw new Error(`${commandType} name "${oldCommandName}" is already exist in command "${removeHomeDir(BlackBot[setMap].get(oldCommandName)?.location || "")}"`);
		}
		// ————————————————— CHECK ALIASES ————————————————— //
		...
		if (configCommand.aliases) {
			...
			for (const alias of aliases) {
				...
				if (BlackBot.aliases.has(alias))
					throw new Error(`alias "${alias}" is already exist in ${commandType} "${BlackBot.aliases.get(alias)}" with file name "${removeHomeDir(BlackBot[setMap].get(BlackBot.aliases.get(alias))?.location || "")}"`);
				BlackBot.aliases.set(alias, scriptName);
			}
		}
		// ————————————————— CHECK ENVCONFIG ————————————————— //
		// env Global
		...
		BlackBot[setMap].delete(oldCommandName);
		BlackBot[setMap].set(scriptName, command);
...
function unloadScripts(folder, fileName, configCommands, getLang) {
	...
	const { BlackBot } = global;
	const { onChat: allOnChat, onEvent: allOnEvent, onAnyEvent: allOnAnyEvent } = BlackBot;
	...
	delete require.cache[require.resolve(pathCommand)];
	BlackBot[setMap].delete(commandName);
