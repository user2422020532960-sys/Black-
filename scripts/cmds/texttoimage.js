const { BlackBotApis } = global.utils;

module.exports = {
	config: {
		name: "نص-إلى-صورة",
		aliases: ["midjourney", "openjourney", "text2image", "texttoimage"],
		version: "1.3",
		author: "Saint",
		countDown: 5,
		role: 0,
	...
	langs: {
		vi: {
			...
			missingGoatApiKey: "❗ Chưa cài đặt apikey cho BlackBot, vui lòng truy cập goatbot.tk để lấy apikey và cài đặt vào file configCommands.json > envGlobal.goatbotApikey và lưu lại"
		},
		en: {
			syntaxError: "⚠️ Please enter prompt",
			error: "❗ An error has occurred, please try again later:\n%1",
			serverError: "❗ Server is overloaded, please try again later",
			missingGoatApiKey: "❗ Not set apikey for BlackBot, please visit goatbot.tk to get apikey and set it to configCommands.json > envGlobal.goatbotApikey and save"
		}
	},

	onStart: async function ({ message, args, getLang, envGlobal }) {
		const goatBotApi = new BlackBotApis(envGlobal.goatbotApikey);
