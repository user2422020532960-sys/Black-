module.exports = {
    ...
    aliases: ["count"],
        ...
        onChat: async ({ usersData, threadsData, event }) => {
                ...
                if (!global._countCache) global._countCache = new Map();
                const cache = global._countCache;

                if (!cache.has(threadID)) {
                        try {
                                const members = await threadsData.get(threadID, "members");
                                cache.set(threadID, { members, dirty: false, timer: null, threadsData });
                        } catch (e) { return; }
                }
                ...
                entry.timer = setTimeout(async () => {
                        ...
                        entry.dirty = false;
                        entry.timer = null;
                        try { await threadsData.set(threadID, entry.members, "members"); } catch (e) {}
                }, 10000);
        }
