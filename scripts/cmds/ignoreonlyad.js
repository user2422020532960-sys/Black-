const ignoreList = global.BlackBot.config.adminOnly.ignoreCommand;
const fs = require("fs-extra");

module.exports = {
	config: {
		name: "تجاهل-وضع-المشرف",
		aliases: ["ignoreadonly", "ignoreonlyadmin", "ignoreadminonly", "ignoreonlyad"],
		version: "1.2",
		author: "Saint",
		countDown: 5,
		role: 2,
	...
	onStart: async function ({ args, message, getLang }) {
		switch (args[0]) {
			case "add": {
				...
				const command = global.BlackBot.commands.get(commandName);
				if (!command)
