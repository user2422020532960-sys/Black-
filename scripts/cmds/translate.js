...
module.exports = {
        ...
        onStart: async function ({ message, event, args, threadsData, getLang, commandName }) {
                if (["-r", "-react", "-reaction"].includes(args[0])) {
                        ...
                        if (global._translateCache) global._translateCache.set(event.threadID, isEnable);
        ...
        onChat: async ({ event, threadsData }) => {
                if (!global._translateCache) global._translateCache = new Map();
                let enabled = global._translateCache.get(event.threadID);
                if (enabled === undefined) {
                        try {
                                enabled = !!(await threadsData.get(event.threadID, "data.translate.autoTranslateWhenReaction"));
                                global._translateCache.set(event.threadID, enabled);
                        } catch { return; }
                }
                if (!enabled) return;
