...
module.exports = {
        ...
        onStart: async function ({ args, threadsData, message, event, role, usersData, getLang, commandName }) {
                ...
                switch (args[0]) {
                        case 'add': {
                                ...
                                if (global._shortcutCache) global._shortcutCache.set(threadID, shortCutData);
        ...
        onReaction: async function ({ event, message, threadsData, getLang, Reaction }) {
                ...
                if (Reaction.type == 'removeAll') {
                        ...
                        if (global._shortcutCache) global._shortcutCache.set(threadID, []);
        ...
        onChat: async ({ threadsData, message, event }) => {
                ...
                if (!global._shortcutCache) global._shortcutCache = new Map();
                let dataShortcut = global._shortcutCache.get(threadID);
                if (dataShortcut === undefined) {
                        try {
                                dataShortcut = await threadsData.get(threadID, 'data.shortcut', []);
                                global._shortcutCache.set(threadID, dataShortcut);
                        } catch { return; }
                }
                if (!dataShortcut || dataShortcut.length === 0) return;
