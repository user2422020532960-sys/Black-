	...
	onStart: async function ({ message, event, args, threadsData, globalData, role, getLang }) {
		...
		switch (args[0]) {
			case "add": {
				...
				if (!global.BlackBot.commands.has(commandName))
				...
				if (args[3] == '-g') {
					if (role > 1) {
						...
						if (global.BlackBot.aliases.has(alias))
							return message.reply(getLang("aliasExist", alias, global.BlackBot.aliases.get(alias)));
						...
						global.BlackBot.aliases.set(alias, commandName);
				...
				if (global.BlackBot.commands.get(alias))
			...
			case "rm": {
				...
				if (args[3] == '-g') {
					if (role > 1) {
						...
						await globalData.set('setalias', globalAliasesData, 'data');
						global.BlackBot.aliases.delete(alias);
